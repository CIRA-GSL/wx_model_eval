...
def _run(eval_file_name_by_period, description_string_by_period,
    ...
    for i in range(num_periods):
        ...
        assert evaluation.ROW_DIM not in evaluation_tables_xarray[i].coords

    first_etx = evaluation_tables_xarray[0]
    target_field_names = first_etx.coords[evaluation.FIELD_DIM].values
    num_target_fields = len(target_field_names)

    # Plot stuff.
    num_examples_by_period = numpy.array([
        numpy.nansum(
            etx[evaluation.RELIABILITY_COUNT_KEY].values, axis=1
        )[0]
        for etx in evaluation_tables_xarray
    ])

    target_mean_matrix = numpy.stack([
        etx[evaluation.TARGET_MEAN_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    target_stdev_matrix = numpy.stack([
        etx[evaluation.TARGET_STDEV_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)
    ...
    for k in range(num_target_fields):
        _plot_one_set_of_metrics_1field(
            ...
            main_y_label_string='Mean ({0:s})'.format(
                FIELD_NAME_TO_UNITS[target_field_names[k]]
            ),
            aux_y_label_string='Stdev ({0:s})'.format(
                FIELD_NAME_TO_UNITS[target_field_names[k]]
            ),
            confidence_level=confidence_level,
    ...
    dwmse_matrix = numpy.stack([
        etx[evaluation.DWMSE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    reliability_matrix = numpy.stack([
        etx[evaluation.RELIABILITY_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)
    ...
    for k in range(num_target_fields):
        _plot_one_set_of_metrics_1field(
            ...
            main_y_label_string='Dual-weighted MSE ({0:s})'.format(
                FIELD_NAME_TO_CUBED_UNITS[target_field_names[k]]
            ),
            aux_y_label_string='Reliability ({0:s})'.format(
                FIELD_NAME_TO_SQUARED_UNITS[target_field_names[k]]
            ),
            confidence_level=confidence_level,
    ...
    mae_matrix = numpy.stack([
        etx[evaluation.MAE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    bias_matrix = numpy.stack([
        etx[evaluation.BIAS_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    mse_matrix = numpy.stack([
        etx[evaluation.MSE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)
    ...
    for k in range(num_target_fields):
        _plot_one_set_of_metrics_1field(
            ...
            main_y_label_string='MAE or RMSE ({0:s})'.format(
                FIELD_NAME_TO_UNITS[target_field_names[k]]
            ),
            aux_y_label_string='Bias ({0:s})'.format(
                FIELD_NAME_TO_UNITS[target_field_names[k]]
            ),
            confidence_level=confidence_level,
    ...
    maess_matrix = numpy.stack([
        etx[evaluation.MAE_SKILL_SCORE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    msess_matrix = numpy.stack([
        etx[evaluation.MSE_SKILL_SCORE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    dwmsess_matrix = numpy.stack([
        etx[evaluation.DWMSE_SKILL_SCORE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    correlation_matrix = numpy.stack([
        etx[evaluation.CORRELATION_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)

    kge_matrix = numpy.stack([
        etx[evaluation.KGE_KEY].values
        for etx in evaluation_tables_xarray
    ], axis=0)
